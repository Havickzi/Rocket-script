-- V7 UNIVERSAL - Работает везде (Delta, Xeno, Krnl, Synapse, Fluxus, Script-Ware)
-- Полный функционал V7 + автодетект инжектора
-- ROCKET версия 7.0 (Universal)

-- ===== АВТО-ДЕТЕКТ ИНЖЕКТОРА =====
local injectorType = "unknown"
local isMobile = false

-- Проверка на мобильный Delta
if syn and syn.crypt then
    injectorType = "Delta (Mobile/PC)"
    isMobile = true
elseif syn and syn.request then
    injectorType = "Synapse X / Delta PC"
elseif http and http.request then
    injectorType = "Krnl / Fluxus"
elseif request then
    injectorType = "Script-Ware / Xeno"
elseif game:GetService("RunService"):IsStudio() then
    injectorType = "Roblox Studio"
else
    injectorType = "Unknown (стандартный режим)"
end

print("🚀 ROCKET V7 Universal | Инжектор: " .. injectorType)
...
local function refreshList()
    ...
    UI.list.CanvasSize = UDim2.new(0, 0, 0, y + 10)
end
...
UI.exec.MouseButton1Click:Connect(function()
    ...
    for i = 1, count do
        if stopFlag then
            UI.counterLabel.Text = "⛔ ОСТАНОВЛЕНО: " .. done .. "/" .. count
            UI.counterLabel.TextColor3 = Color3.fromRGB(255, 100, 100)
            break
        end
        ...
        UI.counterLabel.Text = "⚡ ВЫПОЛНЕНО: " .. done .. " / " .. count
    ...
    if not stopFlag then
        UI.counterLabel.Text = "✅ ГОТОВО! " .. done .. "/" .. count
...
print("🚀 ROCKET V7 Universal загружен. Инжектор: " .. injectorType .. " | КД 5 сек.")
