--[[
    Fable Hub - Premium Interface & Advanced AutoFarm
    Integrated with StealAutoFarm Full Engine:
    - High-Speed Corridor Flight (Z=-360) & Warp Snipe
    - Desync Godmode & Anti-Ragdoll Bypasses
    - Guard Strike Bounce & Re-grab Handling
    - Multi-Zone & Multi-Rarity Targeting Filters (Secret+ Priority)
    - Auto Place (Every 5 Steals) & Auto Hatch Engine
    - Auto Treadmill Mount & Tier Upgrade Engine
    - Auto Speed Trail Purchase & Equip Engine
    - Ultra Potato Mode & 3D Rendering Disabler
    - Anti-AFK & Hide "Not Enough Money" UI Alerts
]]--
...
if parentGui:FindFirstChild("FableHubScreenGui") then
    parentGui.FableHubScreenGui:Destroy()
end

local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "FableHubScreenGui"
...
local function Notify(title, message, duration)
    ...
    u:Create(NotifFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundTransparency = 0.05, Position = UDim2.new(0, 0, 0, 0)}):Play()
    u:Create(UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Transparency = 0.4}):Play()
    u:Create(AccentLine, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundTransparency = 0}):Play()
    u:Create(TitleLabel, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
    u:Create(DescLabel, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {TextTransparency = 0}):Play()
    
    task.delay(duration, function()
        if NotifFrame and NotifFrame.Parent then
            local tweenOut = u:Create(NotifFrame, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {BackgroundTransparency = 1, Position = UDim2.new(1, 40, 0, 0)})
            u:Create(UIStroke, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {Transparency = 1}):Play()
            u:Create(AccentLine, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {BackgroundTransparency = 1}):Play()
            u:Create(TitleLabel, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {TextTransparency = 1}):Play()
            u:Create(DescLabel, TweenInfo.new(0.3, Enum.EasingStyle.Quart, Enum.EasingDirection.In), {TextTransparency = 1}):Play()
            tweenOut:Play()
...
SubtitleLabel.Text = "Fable Hub - Steal An Egg"
SubtitleLabel.TextColor3 = Color3.fromRGB(150, 150, 175)
...
CloseButton.MouseEnter:Connect(function()
    u:Create(CloseButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(220, 60, 60), TextColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = 0}):Play()
end)
CloseButton.MouseLeave:Connect(function()
    u:Create(CloseButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(32, 32, 42), TextColor3 = Color3.fromRGB(200, 200, 215), BackgroundTransparency = 0}):Play()
end)

CloseButton.MouseButton1Click:Connect(function()
    MainFrame.Visible = false
    if ToggleMenuBtn then ToggleMenuBtn.Visible = true end
end)
...
HeaderDivider.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
...
SidebarDivider.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
...
table.insert(connections, w.InputChanged:Connect(function(input)
...
local function CreateCategory(name, iconId)
    ...
    table.insert(connections, CategoryButton.MouseEnter:Connect(function()
        if currentCategory ~= name then
            u:Create(CategoryButton, TweenInfo.new(0.2), {BackgroundTransparency = 0.5}):Play()
        end
    end))

    table.insert(connections, CategoryButton.MouseLeave:Connect(function()
        if currentCategory ~= name then
            u:Create(CategoryButton, TweenInfo.new(0.2), {BackgroundTransparency = 1}):Play()
        end
    end))
    ...
    table.insert(connections, CategoryButton.MouseButton1Click:Connect(function()
        for catName, catData in pairs(categories) do
            if catName == name then
                ...
                u:Create(catData.Button, TweenInfo.new(0.2), {BackgroundTransparency = 0}):Play()
                ...
                u:Create(catData.Button, TweenInfo.new(0.2), {BackgroundTransparency = 1}):Play()
...
local eggFilterPage  = CreateCategory("Egg Selection",  "rbxassetid://10723407874")
...
local function AddToggle(parentPage, title, description, defaultState, callback)
    ...
    ToggleContainer.BackgroundColor3 = Color3.fromRGB(24, 24, 32)
    ...
    Stroke.Color = Color3.fromRGB(42, 42, 55)
    ...
    SwitchBg.BackgroundColor3 = toggled and Color3.fromRGB(130, 85, 255) or Color3.fromRGB(40, 40, 52)
    ...
    local function UpdateToggle(state, noCallback)
        toggled = state
        local targetColor = toggled and Color3.fromRGB(130, 85, 255) or Color3.fromRGB(40, 40, 52)
        local targetPos = toggled and UDim2.new(1, -19, 0.5, -8) or UDim2.new(0, 3, 0.5, -8)
        
        u:Create(SwitchBg, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {BackgroundColor3 = targetColor}):Play()
        u:Create(SwitchKnob, TweenInfo.new(0.2, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {Position = targetPos}):Play()
...
local function AddSlider(parentPage, title, min, max, default, callback)
    ...
    SliderContainer.BackgroundColor3 = Color3.fromRGB(24, 24, 32)
    ...
    SliderTrack.BackgroundColor3 = Color3.fromRGB(40, 40, 52)
    ...
    SliderFill.BackgroundColor3 = Color3.fromRGB(130, 85, 255)
    ...
    table.insert(connections, w.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then
            sliding = false
        end
    end))
...
local function AddButton(parentPage, title, description, callback)
    ...
    ButtonContainer.BackgroundColor3 = Color3.fromRGB(24, 24, 32)
    ButtonContainer.BackgroundTransparency = 0
    ButtonContainer.BorderSizePixel = 0
    ButtonContainer.Parent = parentPage
    ...
    table.insert(connections, ClickButton.MouseEnter:Connect(function()
        u:Create(ButtonContainer, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(34, 34, 46)}):Play()
    end))

    table.insert(connections, ClickButton.MouseLeave:Connect(function()
        u:Create(ButtonContainer, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(24, 24, 32)}):Play()
    end))
...
local function AddDropdown(parentPage, title, options, defaultOption, callback)
    ...
    DropdownContainer.BackgroundColor3 = Color3.fromRGB(24, 24, 32)
    ...
    local TitleLabel = Instance.new("TextLabel")
    TitleLabel.Size = UDim2.new(0, 150, 0, 48)
    ...
    SelectedBtn.Size = UDim2.new(0, 160, 0, 28)
    SelectedBtn.Position = UDim2.new(1, -175, 0, 10)
    ...
    SelectedBtn.TextColor3 = Color3.fromRGB(180, 180, 200)
    ...
    local function RefreshOptions(newOptions)
        ...
        for _, opt in ipairs(newOptions) do
            ...
            OptBtn.MouseButton1Click:Connect(function()
                ...
                SelectedBtn.Text = tostring(selected) .. "  ▼"
                isExpanded = false
                u:Create(DropdownContainer, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 48)}):Play()
                ListHolder.Size = UDim2.new(1, -30, 0, 0)
    ...
    SelectedBtn.MouseButton1Click:Connect(function()
        ...
        if isExpanded then
            ...
            u:Create(DropdownContainer, TweenInfo.new(0.25), {Size = UDim2.new(1, 0, 0, 52 + listHeight)}):Play()
        else
            u:Create(DropdownContainer, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 48)}):Play()
            ListHolder.Size = UDim2.new(1, -30, 0, 0)
        end
...
local function AddMultiDropdown(parentPage, title, options, selectedMap, callback)
    ...
    local function CountSelected()
        ...
        for _, opt in ipairs(options) do
            if selectedMap[opt] == true then c = c + 1 end
        end
        return c
    end
    ...
    SelectedBtn.BackgroundColor3 = Color3.fromRGB(28, 28, 38)
    SelectedBtn.Font = Enum.Font.GothamMedium
    SelectedBtn.Text = tostring(CountSelected()) .. " / " .. tostring(#options) .. "  ▼"
    SelectedBtn.TextColor3 = Color3.fromRGB(180, 180, 200)
    SelectedBtn.TextSize = 12
    SelectedBtn.Parent = DropdownContainer
    ...
    local function UpdateButtonVisual(opt, btn)
    ...
    local function RefreshOptions(newOptions)
        ...
        for _, opt in ipairs(newOptions) do
            ...
            UpdateButtonVisual(opt, OptBtn)

            OptBtn.MouseButton1Click:Connect(function()
                selectedMap[opt] = not (selectedMap[opt] == true)
                UpdateButtonVisual(opt, OptBtn)
                SelectedBtn.Text = tostring(CountSelected()) .. " / " .. tostring(#newOptions) .. "  ▼"
                if callback then pcall(function() callback(selectedMap) end) end
            end)
        end
    end
    ...
    SelectedBtn.MouseButton1Click:Connect(function()
        isExpanded = not isExpanded
        if isExpanded then
            local listHeight = math.clamp(#options * 32, 32, 140)
            ListHolder.Size = UDim2.new(1, -30, 0, listHeight)
            u:Create(DropdownContainer, TweenInfo.new(0.25), {Size = UDim2.new(1, 0, 0, 52 + listHeight)}):Play()
        else
            u:Create(DropdownContainer, TweenInfo.new(0.2), {Size = UDim2.new(1, 0, 0, 48)}):Play()
    ...
    return {
        Get = function() return selectedMap end,
        SetMap = function(newMap)
            selectedMap = newMap
            for opt, btn in pairs(optionButtons) do
                UpdateButtonVisual(opt, btn)
            end
            SelectedBtn.Text = tostring(CountSelected()) .. " / " .. tostring(#options) .. "  ▼"
            if callback then pcall(function() callback(selectedMap) end) end
        end
...
table.insert(connections, y.RenderStepped:Connect(function(dt)
    if tpWalkEnabled then
        local character = o.Character
        if character then
            local rootPart = character:FindFirstChild("HumanoidRootPart")
            if rootPart then
                local cam = r.CurrentCamera
                local moveDir = Vector3.zero
                
                if w:IsKeyDown(Enum.KeyCode.W) then
                    moveDir = moveDir + cam.CFrame.LookVector
                end
                if w:IsKeyDown(Enum.KeyCode.S) then
                    moveDir = moveDir - cam.CFrame.LookVector
                end
                if w:IsKeyDown(Enum.KeyCode.A) then
                    moveDir = moveDir - cam.CFrame.RightVector
                end
                if w:IsKeyDown(Enum.KeyCode.D) then
                    moveDir = moveDir + cam.CFrame.RightVector
                end
...
AddButton(playerPage, "Reset Character", "Teleports character deep into void to trigger server reset", function()
    local character = o.Character
    if character then
        local humanoid = character:FindFirstChildOfClass("Humanoid")
        ...
        if rootPart then
            rootPart.CFrame = CFrame.new(0, -25000, 0)
            rootPart.AssemblyLinearVelocity = Vector3.new(0, -5000, 0)
        else
            character:MoveTo(Vector3.new(0, -25000, 0))
        end
        pcall(function()
            humanoid.Health = 0
        end)
        Notify("Player", "Teleported to void for server reset", 2)
    end
end)

-- ==========================================
-- StealAutoFarm Full Engine (Encapsulated IIFE Module)
-- ==========================================
...
local Z= "FableHub_FlightSpeed.txt"
local z= "FableHub_EggSelectConfig.json"
...
local function bk(...)
    ...
    pcall(function(...)
        ...
        antiAfkConn = o.Idled:Connect(function()
            Ek()
            pcall(function()
                local vu = game:GetService("VirtualUser")
                if vu then
    ...
    task.spawn(function(...)
        while h and (h.alive and h.antiAFK and Lk) do
            Ek()
            for _ = 1, 60 do
...
-- ==========================================
    ...
    return {
        ...
        SetDesyncGodmode = b4,
        ResetFarmStates = D4,
        DoffTreadmillState = C4,
        MountTreadmillOff = M4,
        ExecuteManualPlaceSequence = v4,
        EnableUltraPotatoMode = Mk,
        DisableUltraPotatoMode = Ik,
        EnableAntiAFK = bk,
        DisableAntiAFK = Ak,
        GetTotalEggCount = y4,
        SaveEggSelectConfig = x,
        SaveSpeed = Y,
        ZoneList = M,
        RarityList = X
    }
...
-- ==========================================
-- UI Registration for StealAutoFarm Controls
-- ==========================================

local h = AutoFarm.State

-- Status Dashboard Card in autoFarmPage
local StatusCard = Instance.new("Frame")
StatusCard.Size = UDim2.new(1, 0, 0, 55)
StatusCard.BackgroundColor3 = Color3.fromRGB(24, 24, 32)
StatusCard.BorderSizePixel = 0
StatusCard.Parent = autoFarmPage
...
StatusCardStroke.Color = Color3.fromRGB(120, 80, 255)
StatusCardStroke.Transparency = 0.4
...
StatusCardTitle.Position = UDim2.new(0, 12, 0, 8)
StatusCardTitle.BackgroundTransparency = 1
StatusCardTitle.Font = Enum.Font.GothamBold
StatusCardTitle.Text = "SYSTEM STATUS"
StatusCardTitle.TextColor3 = Color3.fromRGB(140, 90, 255)
StatusCardTitle.TextSize = 11
StatusCardTitle.TextXAlignment = Enum.TextXAlignment.Left
StatusCardTitle.Parent = StatusCard

local StatusCardDesc = Instance.new("TextLabel")
StatusCardDesc.Name = "StatusCardDesc"
StatusCardDesc.Size = UDim2.new(1, -20, 0, 22)
StatusCardDesc.Position = UDim2.new(0, 12, 0, 26)
StatusCardDesc.BackgroundTransparency = 1
StatusCardDesc.Font = Enum.Font.GothamMedium
StatusCardDesc.Text = "Ready"
StatusCardDesc.TextColor3 = Color3.fromRGB(230, 230, 240)
StatusCardDesc.TextSize = 13
StatusCardDesc.TextXAlignment = Enum.TextXAlignment.Left
StatusCardDesc.Parent = StatusCard

task.spawn(function()
    while ScreenGui and ScreenGui.Parent do
        if StatusCardDesc and StatusCardDesc.Parent then
            local carried = AutoFarm.GetTotalEggCount()
            local statusStr = tostring(h.statusText or "Ready")
            if statusStr:sub(1, 1) == "[" then
                StatusCardDesc.Text = string.format("%s • Bag: %d eggs", statusStr, carried)
            else
                StatusCardDesc.Text = string.format("[%s] • Bag: %d eggs", statusStr, carried)
            end
        end
...
-- Auto Steal Mode Controls
AddToggle(autoFarmPage, "Auto Steal (Tween)", "Smoothly fly to steal eggs continuously along Z=-360 highway", h.pureTweenFarm == true, function(state)
    if state then
        AutoFarm.SetFarmMode("TWEEN")
        Notify("AutoSteal", "Tween Farm Started", 2)
    else
        AutoFarm.SetFarmMode("NONE")
        Notify("AutoSteal", "Tween Farm Stopped", 2)
    end
end)

AddToggle(autoFarmPage, "Auto Steal (Teleport)", "Instantly warp to steal target eggs in rapid loop", h.autoFarmLoop == true, function(state)
    if state then
        AutoFarm.SetFarmMode("WARP")
        Notify("AutoSteal", "Teleport Farm Started", 2)
    else
        AutoFarm.SetFarmMode("NONE")
        Notify("AutoSteal", "Teleport Farm Stopped", 2)
    end
end)

AddToggle(autoFarmPage, "Auto Return", "Automatically fly back to safe line (X=525) after stealing", h.autoGlide ~= false, function(state)
    h.autoGlide = state
    Notify("AutoSteal", "Auto Return set to: " .. tostring(state), 2)
end)

AddToggle(autoFarmPage, "Auto Place (Every 5)", "Return home every 5 steals to deposit eggs to incubators", h.autoPlaceEvery5 == true, function(state)
    h.autoPlaceEvery5 = state
    if not state then h.batchStealCount = 0 end
    Notify("AutoPlace", "Auto Place (Every 5) set to: " .. tostring(state), 2)
end)

AddToggle(autoFarmPage, "Auto Hatch", "Continuously hatch ready eggs automatically from anywhere", h.autoHatch ~= false, function(state)
    h.autoHatch = state
    Notify("AutoHatch", "Auto Hatch set to: " .. tostring(state), 2)
end)

AddButton(autoFarmPage, "Place Carried Eggs Now", "Fly home immediately, place stashed eggs into stands, and hatch", function()
    Notify("AutoPlace", "Placing carried eggs...", 2)
    task.spawn(function()
        AutoFarm.ExecuteManualPlaceSequence(h.glideSpeed, nil, true)
    end)
end)

AddToggle(autoFarmPage, "Auto Treadmill", "Run on base treadmill when no target eggs are spawned", h.autoTreadmill ~= false, function(state)
    h.autoTreadmill = state
    AutoFarm.SaveEggSelectConfig(h)
    Notify("Treadmill", "Auto Treadmill set to: " .. tostring(state), 2)
end)

AddToggle(autoFarmPage, "Auto Upgrade Treadmill", "Automatically upgrade base treadmill tier when you have enough cash", h.autoUpgradeTreadmill ~= false, function(state)
    h.autoUpgradeTreadmill = state
    AutoFarm.SaveEggSelectConfig(h)
    Notify("Treadmill", "Auto Upgrade set to: " .. tostring(state), 2)
end)

AddToggle(autoFarmPage, "Auto Buy & Equip Trails", "Automatically purchase and equip best available speed trail", h.autoBuyTrails ~= false, function(state)
    h.autoBuyTrails = state
    AutoFarm.SaveEggSelectConfig(h)
    Notify("Trails", "Auto Buy Trails set to: " .. tostring(state), 2)
end)

AddToggle(autoFarmPage, "Hide 'Not Enough Money' UI", "Automatically suppress red 'Not enough money' game alert", h.hideNotEnoughMoney ~= false, function(state)
    h.hideNotEnoughMoney = state
    AutoFarm.SaveEggSelectConfig(h)
    Notify("UI", "Hide Alert set to: " .. tostring(state), 2)
end)

-- Egg Filter Category UI Controls
AddMultiDropdown(eggFilterPage, "Target Zones", AutoFarm.ZoneList, h.selectedZones, function(selectedMap)
    h.selectedZones = selectedMap
    AutoFarm.SaveEggSelectConfig(h)
end)

AddMultiDropdown(eggFilterPage, "Target Rarities", AutoFarm.RarityList, h.selectedRarities, function(selectedMap)
    h.selectedRarities = selectedMap
    AutoFarm.SaveEggSelectConfig(h)
end)

AddToggle(eggFilterPage, "Always Steal Secret+ Eggs", "Collect Secret, Eternal, Divine eggs from any zone automatically", h.alwaysCollectSecretPlus ~= false, function(state)
    h.alwaysCollectSecretPlus = state
    AutoFarm.SaveEggSelectConfig(h)
    Notify("EggFilter", "Always Secret+ set to: " .. tostring(state), 2)
end)

-- Movement & Flight Category Additions
AddSlider(movementPage, "Flight Cruise Speed", 100, 1000, h.glideSpeed or 600, function(val)
    h.glideSpeed = val
    AutoFarm.SaveSpeed(val)
end)

AddToggle(movementPage, "Desync Godmode", "Full immunity against map obstacles, traps, and hazards", h.godmode == true, function(state)
    AutoFarm.SetDesyncGodmode(state)
    Notify("Safety", "Godmode set to: " .. tostring(state), 2)
end)

AddButton(movementPage, "Get Unstuck / Get Off Treadmill", "Instantly break free from treadmills, seats, or map geometry", function()
    AutoFarm.ResetFarmStates()
    AutoFarm.DoffTreadmillState()
    AutoFarm.MountTreadmillOff()
    Notify("Safety", "Unstuck executed!", 2)
end)

-- Settings Category Additions
AddToggle(settingsPage, "Ultra Potato Mode (Max FPS)", "Disables textures, meshes, lights, shadows & particles for max FPS", h.performanceMode == true, function(state)
    if state then
        AutoFarm.EnableUltraPotatoMode()
        Notify("Performance", "Ultra Potato Mode Enabled", 2.5)
    else
        AutoFarm.DisableUltraPotatoMode()
        Notify("Performance", "Ultra Potato Mode Disabled", 2.5)
    end
    AutoFarm.SaveEggSelectConfig(h)
end)

AddToggle(settingsPage, "Disable 3D Rendering (GPU Saver)", "Freezes 3D viewport to drop GPU usage to ~1% for overnight farm", h.disable3D == true, function(state)
    h.disable3D = state
    pcall(function()
        y:Set3dRenderingEnabled(not state)
    end)
    AutoFarm.SaveEggSelectConfig(h)
    Notify("Performance", "3D Rendering set to: " .. tostring(not state), 2.5)
end)

AddToggle(settingsPage, "Anti-AFK", "Prevents AFK disconnect (Idled Event + VirtualUser + Key Pulse)", h.antiAFK ~= false, function(state)
    h.antiAFK = state
    if state then
        AutoFarm.EnableAntiAFK()
    else
        AutoFarm.DisableAntiAFK()
    end
    AutoFarm.SaveEggSelectConfig(h)
    Notify("System", "Anti-AFK set to: " .. tostring(state), 2.5)
end)

if h.antiAFK ~= false then
    task.spawn(function()
        task.wait(1)
        AutoFarm.EnableAntiAFK()
    end)
...
AddButton(settingsPage, "Unload Script", "Completely terminate all farm loops and close interface", function()
    h.alive = false
    AutoFarm.SetFarmMode("NONE")
    AutoFarm.ResetFarmStates()
    pcall(function()
    ...
    Notify("System", "Script Unloaded", 2)
end)
